...
const ResultCardComponent: React.FC<ResultCardProps> = ({ icon, label, status, onClick }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="font-semibold text-lg text-gray-900 dark:text-gray-100">{label}</span>
      </div>
      <div className={`px-3 py-1 rounded-full text-sm font-bold ${color}`}>{text}</div>
