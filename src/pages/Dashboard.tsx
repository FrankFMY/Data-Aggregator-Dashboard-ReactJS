...
const MODES: { value: PromiseMode; label: string; description: string }[] = [
  {
    ...
    description: 'Все запросы выполняются одновременно. Ошибка любого — ошибка всей группы.',
  },
  {
    value: 'sequential',
  ...
  },
  {
    ...
    description: 'Результат — первый успешно завершившийся запрос. Остальные отменяются.',
  },
  {
    value: 'allSettled',
...
export const Dashboard = () => {
  ...
  const fetchers = useMemo(
    () =>
      apiOptions.filter((opt) => selected.includes(opt.key as ApiKey)).map((opt) => opt.fetcher),
    [selected],
  );

  const { results, run, cancel, progress, status } = usePromiseManager(fetchers, {
    mode,
    retry: 2,
    timeout: 7000,
  });
      ...
      <div className="w-full max-w-3xl bg-white/90 dark:bg-gray-900/90 rounded-3xl shadow-2xl p-6 md:p-10 border border-gray-200 dark:border-gray-800 backdrop-blur-sm">
            ...
            {apiOptions.map((opt) => (
              <label
                ...
                className={`flex items-center gap-2 cursor-pointer px-3 py-2 md:px-4 md:py-2 rounded-lg transition bg-gray-50 dark:bg-gray-700 hover:bg-blue-50 dark:hover:bg-gray-600 border border-transparent hover:border-blue-200 dark:hover:border-gray-600 shadow-sm ${
                  selected.includes(opt.key as ApiKey) ? 'ring-2 ring-blue-300' : ''
                }`}
              >
                <input
                  type="checkbox"
                  checked={selected.includes(opt.key as ApiKey)}
                  onChange={() => handleSelect(opt.key as ApiKey)}
        ...
        <div className="mb-8 flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              <span className="text-gray-700 dark:text-gray-200 font-medium text-sm">Режим:</span>
        ...
        {modalIdx !== null && results[modalIdx] && (
          <Modal
            ...
            copyContent={
              results[modalIdx].data ? JSON.stringify(results[modalIdx].data, null, 2) : undefined
            }
          >
            {results[modalIdx].loading && <Skeleton height="2rem" />}
            {results[modalIdx].error && <ErrorBlock error={results[modalIdx].error} />}
            {results[modalIdx].data && (
              <pre className="bg-gray-100 dark:bg-gray-800 rounded p-4 w-full text-xs text-gray-800 dark:text-gray-100 overflow-x-auto max-h-96 whitespace-pre-wrap">
                {JSON.stringify(results[modalIdx].data, null, 2)}
              </pre>
            )}
