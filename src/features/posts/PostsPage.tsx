...
const PostsPage = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      {posts && <PostsList posts={posts} onPostClick={setSelectedPost} />}
      {!posts && !loading && (
        <div className="text-gray-400 text-center mt-8">Нет данных. Нажмите «Загрузить посты».</div>
      )}
      {selectedPost && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="post-modal-content">
            <PostModal post={selectedPost} onClose={handleModalClose} />
          </div>
        </div>
      )}
    </div>
